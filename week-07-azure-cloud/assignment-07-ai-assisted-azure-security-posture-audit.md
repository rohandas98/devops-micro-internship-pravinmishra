...
![Week7 screenshots](screenshots/Assignment7/Screenshot1.png)
...
![Week7 screenshots](screenshots/Assignment7/Screenshot2.png)
...
![Week7 screenshots](screenshots/Assignment7/Screenshot3.png)
...
![Week7 screenshots](screenshots/Assignment7/Screenshot4.png)

---

#### Screenshot 5 — Output of `bash -n` (no syntax errors) and `ls -l` showing the script is executable

![Week7 screenshots](screenshots/Assignment7/Screenshot5.png)
...
![Week7 screenshots](screenshots/Assignment7/Screenshot6.png)
...
![Week7 screenshots](screenshots/Assignment7/Screenshot7.png)

---

#### Screenshot 8 — `/azure-audit` output showing the baseline findings and Claude's explanation

![Week7 screenshots](screenshots/Assignment7/Screenshot8.png)
...
![Week7 screenshots](screenshots/Assignment7/Screenshot9.png)

---

#### Screenshot 10 — Terminal output of the remediation command you ran yourself

![Week7 screenshots](screenshots/Assignment7/Screenshot10.png)

---

#### Screenshot 11 — Second `/azure-audit` run (or report) showing the finding resolved

![Week7 screenshots](screenshots/Assignment7/Screenshot11.png)
...
## AWS vs Azure Audit: Reflection

Doing the Week 7 Azure audit felt like retracing Week 6's steps with different street signs. The question never changed — "is this exposed, misconfigured, or over-permissioned?" — only the vocabulary did.

**Finding Category Mapping**

| AWS (Week 6) | Azure (Week 7) | What's actually being checked |
|---|---|---|
| EC2 | Virtual Machines | Compute exposed to the internet unnecessarily |
| Security Groups | NSGs | Overly permissive inbound/outbound rules |
| S3 | Storage Accounts/Blob Storage | Publicly readable/writable data |
| RDS | Azure database services | DB reachable from outside the network, exposed creds |
| Access & Exposure | Access & Exposure | Who *can* touch a resource vs. who *should* |

Every category boils down to the same five things — compute, network boundary, data at rest, data behind a service, and who holds the keys — just named differently per provider.

**What Stayed the Same**

**Gather → Analyze → Recommend → Human Approves → Execute → Verify**

- **Gather**: `aws ec2 describe-instances` ↔ `az vm list` — same intent, different syntax
- **Analyze**: the security logic (public exposure, open ports, unencrypted storage = bad) is cloud-agnostic
- **Recommend**: remediation reasoning doesn't change across providers
- **Human Approves**: deliberately provider-independent — no CLI difference should skip human sign-off before touching live infra
- **Execute**: commands differ, but it's just the mechanism, not the decision
- **Verify**: re-running gather to confirm the fix landed works the same way on both

**Takeaway**: the commands are a thin, swappable layer. The audit logic, risk model, and human-in-the-loop gate are portable across clouds — suggesting the same approach could extend to GCP with similar changes.
